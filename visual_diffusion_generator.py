...
def load_model_for_visualization(checkpoint_path: str):
    """Load model and tokenizer for visualization with same logic as web interface"""
    ...
    # Check if config exists and has required keys (same as web interface)
    if 'config' not in checkpoint or 'd_model' not in checkpoint.get('config', {}):
        console.print("[yellow]⚠️  Invalid config in checkpoint, creating default config...[/yellow]")
        ...
        model, _ = load_model_checkpoint(checkpoint_path, 'cpu')
